...
def login():
    if request.method == 'POST':
        ...
        if not user:
            flash('invaild username. Please sign up below') 
            return redirect ('/login')

        if user and user.password != password:
            flash('invaild password, The password you entered is incorrect.')
...
def signup():
    if request.method == 'POST':
        ...
        if len(username) < 1:
            flash("No username found. try again. ")
        if len(password) < 1:
            flash("are you dumb...Invalid password")
        if len(verify) <1:
            flash("Cant leave this blank either.")
            return redirect ('/signup')
        if password != verify:
            flash('passwords did not match whats on file')
            return redirect('/signup')
...
def blog():
    ...
    if (blog_id):
        ...
        return render_template('blogentry2.html', blog=blog)
...
def add_blog():
    ...
    if request.method == 'POST':
        ...
        if len(blog_title) < 1:
            title_error = "don't forgot to title this blog!"
            blog_title=""
     
        if len(blog_body) < 1:
            body_error = "whats your blog about?"
            blog_body =""
